      ...
      if (game_has_error(g, i, j)) {
        if (game_is_black(g, i, j)) {
          ...
          if (black_number != -1) {
            ...
            sq_aux.w = square.w / 1.3;
            sq_aux.h = square.h / 1.3;
            if (black_number == 0) SDL_RenderCopy(ren, env->red0, NULL, &sq_aux);
            if (black_number == 1) SDL_RenderCopy(ren, env->red1, NULL, &sq_aux);
            if (black_number == 2) SDL_RenderCopy(ren, env->red2, NULL, &sq_aux);
            if (black_number == 3) SDL_RenderCopy(ren, env->red3, NULL, &sq_aux);
            if (black_number == 4) SDL_RenderCopy(ren, env->red4, NULL, &sq_aux);
          }
        } else {
...
bool process(SDL_Window* win, SDL_Event* e, game g)
{
  ...
  if (e->type == SDL_QUIT) {
    ...
    /* else if we click at the bottom of the grid */
      ...
      if (BottomButtonsActions(g, &mouse, &game_grid, &bottom_buttons_rect)) return true;
    }
